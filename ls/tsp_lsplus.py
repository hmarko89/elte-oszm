...
from tsp_ls import random_euclidean_graph, __evaluate_solution
...
def tabu_search( graph:nx.DiGraph, max_iterations:int= 100, tabu_length:int= 10, draw_progress:bool= False ) -> list:
    ...
    Args:
        - graph:          networkx digraph (with 'cost' edge attributes)
        - max_iterations: maximum number of iterations
        - tabu_length:    length of the tabu list
    ...
    best_global_cost = __evaluate_solution( graph, curr_solution )
    ...
    for iterations in range(max_iterations):
        ...
        for (i,j) in it.combinations(range(len(curr_solution)),2):
            ...
            neighbor_cost = __evaluate_solution( graph, neighbor_solution )
...
def simulated_annealing( graph:nx.DiGraph, temperature:float, cooling_rate:float, draw_progress:bool = False ) -> list[tuple[int,int]]:
    ...
    Args:
        ...
        - cooling_rate:  rate at which temperature is decreased
        - draw_progress: should we draw the cost evolution over iterations?
    ...
    # create primitive initial solution
    curr_solution = list(graph.nodes)
    curr_cost = __evaluate_solution( graph, curr_solution )
    ...
    while temperature > 0.1:
        ...
        new_cost = __evaluate_solution( graph, new_solution )
...
def genetic_algorithm( graph:nx.DiGraph, population_size:int, generations:int, mutation_rate:float, draw_progress:bool= False ) -> list[tuple[int,int]]:
    ...
    population.sort( key= lambda individual: __evaluate_solution( graph, individual ) )
    ...
    for generation in range(generations):        
        ...
        # adjust population
        new_population.sort( key= lambda individual: __evaluate_solution( graph, individual ) )
        population = new_population[:population_size]
    
        best_costs.append( __evaluate_solution( graph, population[0] ) )
        worst_costs.append( __evaluate_solution( graph, population[-1] ) )
